...
var userMod = module.exports = function (connStr) {
	var deferred = when.defer();
	pg.connect(connStr, function (err, client) {
		deferred.resolver.resolve(client);
	});
	this.pgConn = deferred.promise;
};

userMod.prototype.getUser = function (sessionID) {
	var deferred = when.defer();
	when(this.pgConn,
		function (client) {
			client.query('select users.get_user($1)', [sessionID], 
				function (err, result) {
					deferred.resolver.resolve(result.rows[0].get_user);
				}
			);
		}
	);
...
userMod.prototype.login = function (sessionID, username, password) {
	var deferred = when.defer();
	when(this.pgConn,
		function (client) {
